...
class DynTanh(nn.Module):
    def __init__(self, dims, init_alpha=1.0):
        super(DynTanh, self).__init__()
...
class MultiHeadAttention(nn.Module):
    def __init__(self, d_model, n_heads):
        ...
        self.layer_norm = nn.LayerNorm(d_model) # DynTanh(d_model)  #
...
class Encoder(nn.Module):
    def __init__(self, features_dim, dfeatures_dim, hidden_size, args):
        ...
        self.L1 = nn.Linear(features_dim, hidden_size // 2)  # for static features embedding
        self.L2 = nn.Linear(dfeatures_dim, hidden_size // 2)  # for dynamic features embedding

        self.layers = nn.ModuleList([EncoderLayer(hidden_size, d_ff, n_heads) for _ in range(n_layers)])

    def forward(self, emb_inp, rec_inp, adj_mask, dummy_arg):
        for layer in self.layers:
            emb_inp, _ = layer(emb_inp, rec_inp, adj_mask)
...
class Decoder(nn.Module):
    ...
    def forward(self, inputs, hidden, enc_outputs, mask):
        ...
        hidden = self.lstm(inputs, hidden)
        hidden_state = hidden[0]
        w2h = self.W2(hidden_state).unsqueeze(1)
        u = torch.tanh(w1e + w2h)
        a = u.matmul(self.V)  # [b_size x len_q x 1]
        a = self.clip * torch.tanh(a).squeeze(2)  # [-10, 10]
        a = a.masked_fill(mask == 0, -np.inf)  # fill masked values with -inf
        policy = F.softmax(a, dim=1)
...
class RecPointerNetwork(nn.Module):
    """
    Recursion Pointer Network architecture.
    """
    ...
    def forward(self, enc_inputs, enc_hidden, adj_mask, dec_input, dec_hidden, mask, first_step=False):
        policy, dec_hidden, enc_outputs = self._one_step(enc_inputs, enc_hidden, adj_mask, dec_input, dec_hidden, mask,
                                                         first_step)
